...
import { AlertTriangle, Users, Trash2, Plus, Clock, MapPin, Building2 } from "lucide-react";
...
const validarFechas = (fechaAlta?: string | null, fechaBaja?: string | null): string[] => {
  ...
  if (fechaAlta && fechaBaja) {
    ...
    if (alta > baja) {
      errores.push("La fecha de alta no puede ser posterior a la fecha de baja");
    }
  }
...
const PrestadoresFormEdit: React.FC<PrestadoresFormEditProps> = ({ prestador }) => {
  ...
  const [profesionalesIndependientes, setProfesionalesIndependientes] = useState<Prestador[]>([]);
  const [profesionalesAsociados, setProfesionalesAsociados] = useState<Prestador[]>([]);
  ...
  useEffect(() => {
    if (prestador) {
      ...
      setTipoPrestador(prestador.esProfesionalIndependiente ? "Profesional Independiente" : "Centro Médico");
  ...
  useEffect(() => {
    if (prestador?.id && !prestador.esProfesionalIndependiente) {
      fetch(getApiUrl(`/prestadores/${prestador.id}/profesionales`))
        ...
        .catch(err => console.error("Error cargando profesionales asociados:", err));
    } else {
      setProfesionalesAsociados([]);
    }
  }, [prestador]);
  ...
  useEffect(() => {
    if (isEditing && tipoPrestador === "Centro Médico") {
      fetch(getApiUrl("/prestadores"))
        .then(res => res.json())
        .then((data: Prestador[]) => {
          const profesionales = data.filter(p => p.esProfesionalIndependiente);
  ...
  const handleAgregarProfesional = async () => {
    ...
    try {
      const res = await fetch(
        getApiUrl(`/prestadores/${prestador.id}/profesionales/${profesionalSeleccionado}`),
        { method: "POST" }
      );
      ...
      if (profesionalAgregado) {
        ...
        modal.mostrarModal({
          ...
          submensaje: "Las direcciones del centro han sido agregadas al profesional.",
          tipo: "success",
      ...
      if (mensajeError.includes("especialidad en común")) {
        detalles = ["El profesional debe tener al menos una especialidad que coincida con las del centro médico."];
  ...
  const handleEliminarProfesional = async (profesionalId: number) => {
    ...
    const profesional = profesionalesAsociados.find(p => p.id === profesionalId);
    
    modal.mostrarModal({
      titulo: "Desvincular profesional",
      mensaje: `¿Está seguro que desea desvincular a ${profesional?.nombreCompleto || 'este profesional'} del centro médico?`,
      submensaje: "Se eliminarán las direcciones del centro médico del profesional.",
      tipo: "warning",
      ...
      onConfirmar: async () => {
        ...
        try {
          const res = await fetch(
            getApiUrl(`/prestadores/${prestador.id}/profesionales/${profesionalId}`),
            { method: "DELETE" }
          );
          ...
          setProfesionalesAsociados(prev => prev.filter(p => p.id !== profesionalId));
          
          modal.mostrarModal({
            titulo: "✅ Profesional desvinculado",
            mensaje: "Se ha desvinculado el profesional del centro médico exitosamente.",
            tipo: "success",
            textoBotonConfirmar: "Aceptar",
            soloInformacion: true
          });
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      modal.mostrarModal({
        ...
        mensaje: `Se ha ${prestador ? "actualizado" : "creado"} el prestador ${nombre} correctamente.`,
        tipo: "success",
        textoBotonConfirmar: "Aceptar",
        soloInformacion: true,
        onConfirmar: () => {
  ...
  return (
    <>
      <div className="prestador-form">
          ...
          <div className="form-row" style={{ flexDirection: "column", gap: "1rem" }}>
            ...
            {profesionalesAsociados.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {profesionalesAsociados.map((prof) => (
                  <div
                    key={prof.id}
                    style={{
                      display: "flex",
                    ...
                    <div>
                      ...
                      {prof.especialidades?.length > 0 && (
                        <div style={{ fontSize: "0.85rem", color: "#666", marginTop: "0.25rem" }}>
                          {prof.especialidades.map(e => e.nombre).join(", ")}
                        </div>
                      )}
                    </div>
                    {isEditing && (
                  ...
                  <Select
                    options={[
                      ...
                      ...profesionalesDisponibles.map(p => ({
                        value: p.id.toString(),
                        label: `${p.nombreCompleto} - ${p.especialidades?.map(e => e.nombre).join(", ") || "Sin especialidades"}`
                      }))
                    ]}
            ...
            {isEditing && profesionalesDisponibles.length === 0 && profesionalesIndependientes.length > 0 && (
        ...
        {tipoPrestador === "Profesional Independiente" && prestador && centrosMedicos.length > 0 && (
          <div className="form-row" style={{ flexDirection: "column", gap: "1rem" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              <Building2 size={20} />
            ...
            {loadingCentros ? (
              <p style={{ color: "#666", fontStyle: "italic" }}>Cargando centros médicos...</p>
            ) : (
                ...
                {centrosMedicos.map((centro) => (
                  ...
                  >
                    <div style={{ 
                      ...
                      <strong style={{ fontSize: "1.05rem", color: "#1565c0" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        {centro.direccion.map((dir, idx) => (
                          ...
                          >
                            ...
                            {dir.horariosAtencion && dir.horariosAtencion.length > 0 ? (
                                ...
                                {dir.horariosAtencion.map((hor, hidx) => (
                                  ...
                                  >
                                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                      <Clock size={14} />
                                      <span>
                                        <strong>{hor.dia}</strong>: {hor.desde} - {hor.hasta}
                                      </span>
                                    </div>
                              ...
                              <p style={{ fontSize: "0.85rem", color: "#999", fontStyle: "italic", marginLeft: "1.5rem" }}>
        ...
        <ModalConfirmacion
          ...
          submensaje={tipoBaja === "inmediata" ? "Se dará de baja inmediatamente (hoy)" : `Se dará de baja el ${fechaBajaSeleccionada}`}
        ...
        {modalBajaOpen && (
          <div style={{
            ...
            <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
              <Button variant="cancel" size="small" onClick={() => setModalBajaOpen(false)}>
                Cancelar
              </Button>
              <Button variant="danger" size="small" onClick={handleConfirmarBaja}>
                Confirmar baja
              </Button>
            </div>
          </div>
